...
for i=1:size(traces1,1)
    ...
    trimTimepoints(1) = max(find(~isnan(currentTrace1), 1, 'first'), find(~isnan(currentTrace2), 1, 'first'));
    trimTimepoints(2) = min(find(~isnan(currentTrace1), 1, 'last'), find(~isnan(currentTrace2), 1, 'last'));
